...
import { getDateRange } from "@/modules/functions";
  ...
  return (
    <div style={{ marginTop: "70px" }}>
      <div className="container-fluid d-sm-flex justify-content-center pt-1">
        <input
          className="form-control w-auto mx-auto mb-2 m-sm-0 me-sm-3"
          type="search"
          placeholder="Search"
          aria-label="Search"
          defaultValue={search}
          onChange={(e) => setSearch(e.target.value.substring(0, 100))}
        />
      ...
      <InfiniteScroll
        className="panel row mt-3 mx-3 py-2 gx-4"
        next={fetchNextPage}
        hasMore={hasNextPage}
        loader={isFetching && <Loader />}
        endMessage={length > 0 && <p className="text-center fw-bold">Yay! You have seen it all</p>}
        dataLength={length}
      >
        {length ? (
          news.map((item, i) => (
