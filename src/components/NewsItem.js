...
export default function NewsItem(props) {
    ...
    if (imgUrl?.match(/http/g)?.length !== 1) var newsImg = '/news.webp'
    else {
        const connectionSpeed = navigator.connection?.effectiveType
        newsImg = `https://images.weserv.nl/?url=${imgUrl}&width=450&height=300&maxage=1d&output=webp&q=${connectionSpeed?.includes('2') ? 5 : connectionSpeed?.includes('3') ? 10 : 25}`
    }
    ...
    return <div className="card mt-4 mb-3" style={{ paddingBottom: "2rem" }}>
        ...
        <a href={newsUrl} target="_blank" rel="noreferrer" className="text-black">
            <div style={{ height: "13rem" }}>
                <img src={newsImg} loading='lazy' className="card-img-top h-100" alt='' />
            </div>
        </a>
