...
export default function News() {
    ...
    const queryKey = [code, category]
    const id = 'news' + queryKey.join('')
    ...
    const { isLoading, data, hasNextPage, fetchNextPage } = useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 1 }) => {
            ...
            const { data: { success, nextPage, news } } = await axios({
                ...
                data: { country: code, category: category || 'general', page: pageParam }
            })
    ...
    return <div style={{ marginTop: "70px" }}>
        ...
        <hr />
        <InfiniteScroll className="panel row mx-3 py-2 gx-4" next={fetchNextPage} hasMore={hasNextPage} loader={isLoading && <Loader />} endMessage={news.length > 0 && <p className='text-center fw-bold'>Yay! You have seen it all</p>} dataLength={fullNews.length}>
