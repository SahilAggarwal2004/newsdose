            ...
            <div className="container-fluid">
                ...
                <div className="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul className="navbar-nav pe-3 me-auto mb-2 mb-lg-0">
                        {categories.map(element => <li className="nav-item" key={element} onClick={() => resetNews(element)}>
                            <Link className="nav-link d-inline-block w-auto" aria-current="page" to={`/${element}`}>
                                <button className='btn shadow-none nav-link p-0 text-capitalize' data-bs-toggle='collapse' data-bs-target={width > 991 ? '' : "#navbarSupportedContent"}>{element || "Home"}</button>
                            </Link>
                        </li>)}
                    </ul>
