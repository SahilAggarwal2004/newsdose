...
export default function Search() {
    const { country: { method, code }, setProgress, error, onError } = useNewsContext()
    ...
    now.setDate(now.getDate() - 6); // 7 days
    const minDate = now.toLocaleDateString('en-ca');
    const queryKey = [code, category, query, date]
    const id = search + queryKey.join('')
    ...
    const { isLoading, data, hasNextPage, fetchNextPage } = useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 1 }) => {
            ...
            const { data: { success, nextPage, news } } = await axios({
                url: process.env.REACT_APP_URL + 'search', method: 'post',
                headers: { accesstoken: encrypt(Date.now()), 'Content-Type': 'application/json' },
                data: { country: code, category: category || 'general', page: pageParam, query, date }
            })
    ...
    return <div style={{ marginTop: "70px" }}>
        ...
        <InfiniteScroll className="panel row mt-3 mx-3 py-2 gx-4" next={fetchNextPage} hasMore={hasNextPage} loader={isLoading && <Loader />} endMessage={news.length > 0 && <p className='text-center fw-bold'>Yay! You have seen it all</p>} dataLength={news.length}>
